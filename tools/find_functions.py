...
def _is_padding(insn: Instruction) -> bool:
    """Inter-function padding: nop (ori r0,r0,0) or a zero/illegal word."""
    return insn.mnemonic in ("nop", ".word", "illegal", "udi") or (
        insn.mnemonic == "ori" and insn.operands.replace(" ", "") == "r0,r0,0")


def _is_prologue_start(insns: list, idx: int, n: int) -> bool:
    """True if insns[idx] begins a standard or frame-first function prologue.
    Used to decide whether a `blr` is a real function boundary (next thing is a
    new function) vs an early-return inside a multi-return function."""
    if idx >= n:
        return False
    cur = insns[idx]
    # standard: mflr r0 ; ... store-lr ... ; ... stdu ...
    if _is_mflr_r0(cur):
        has_store = any(_is_store_lr(insns[m]) for m in range(idx + 1, min(idx + 4, n)))
        has_alloc = any(_is_stack_alloc(insns[m]) for m in range(idx + 1, min(idx + 7, n)))
        if has_store and has_alloc:
            return True
    # frame-first: stdu r1,-X(r1) ; ... mflr r0 ...
    if _is_stack_alloc(cur):
        if any(_is_mflr_r0(insns[m]) for m in range(idx + 1, min(idx + 7, n))):
            return True
    return False
...
class FunctionFinder:
    ...
    def find_prologue_functions(self) -> None:
        ...
        while i < n:
            ...
            # A genuine prologue saves LR (found_store) AND establishes a frame
            # (stdu) right around the mflr: either just after it (standard
            # `mflr; std r0,X(r1); stdu r1,-Y(r1)`) or just before it
            # (frame-first `stdu; ...; mflr`). A bare `mflr r0; std r0,X(r1)`
            # with NO nearby stdu is a *mid-function* LR save (a function that
            # makes calls partway through), not an entry — anchoring a function
            # there (e.g. 0x113654 inside func_0011363C) clips the real seeded
            # function short and the sliver removal then orphans the tail,
            # producing a wrong fallthrough and an infinite loop at runtime.
            alloc_before = i > 0 and _is_stack_alloc(self.instructions[i - 1])
            if not found_store or not (found_stack or alloc_before):
                i += 1
                continue
            ...
            for j in range(i + 1, n):
                ...
                if _is_blr(curr):
                    ...
                    # A `blr` only ENDS the function if what follows is a real
                    # boundary (a new prologue, inter-function padding, or EOF).
                    # Optimized routines (memcpy/strcpy with early-exit blrs,
                    # e.g. func_00D70BFC) have basic blocks placed AFTER an early
                    # blr, reached by later branches; ending at the first blr
                    # orphaned them -> a callee that fails to restore r31 ->
                    # corrupted callee-saved regs + mis-terminated BSS tables.
                    # Keep scanning past an early-return blr; overlap-clipping to
                    # seeded/OPD starts is the backstop against over-extension.
                    nxt = self.instructions[j + 1] if j + 1 < n else None
                    if (nxt is None or _is_padding(nxt)
                            or _is_prologue_start(self.instructions, j + 1, n)):
                        func_end = curr.addr + 4
                        break

                # Safety: if we hit another *standard* prologue, stop. A bare
                # `mflr r0` is NOT enough: PPC64 ELFv1 frequently uses a
                # frame-first prologue (`stdu r1,-Y(r1); ...; mflr r0; std
                # r0,X(r1)`) where the mflr sits well inside the function (e.g.
                # func_0011363C has it at +0x18). Treating that mid-function
                # mflr as a new boundary truncated the body and left the real
                # code orphaned -> wrong fallthrough -> infinite loop. Only
                # break on a genuine standard prologue: mflr r0 immediately
                # followed by a store-LR AND a stack-alloc shortly after (the
                # frame-first variant has its stdu *before* the mflr, so it
                # fails this test and is correctly skipped).
                if j > i + 2 and _is_mflr_r0(curr):
                    has_store_lr = any(_is_store_lr(self.instructions[m])
                                       for m in range(j + 1, min(j + 4, n)))
                    has_alloc = any(_is_stack_alloc(self.instructions[m])
                                    for m in range(j + 1, min(j + 7, n)))
                    if has_store_lr and has_alloc:
                        func_end = curr.addr
                        break
            ...
            for j in range(idx, min(idx + 200, len(self.instructions))):
                ...
                if _is_bl(curr):
                    ...
                    if t is not None:
                        ...
                        func.is_leaf = False
                if _is_blr(curr):
                    # Multi-return leaf (memcpy/strcpy with early blrs, e.g.
                    # func_00D70BFC): only end at a real boundary, else keep
                    # scanning (still bounded by the next-known-start check
                    # below and the 200-insn cap). Ending at the first blr
                    # truncated these and orphaned their later basic blocks.
                    nx = self.instructions[j + 1] if j + 1 < len(self.instructions) else None
                    if (nx is None or _is_padding(nx) or (nx.addr in self.functions)
                            or _is_prologue_start(self.instructions, j + 1, len(self.instructions))):
    ...
    def apply_seeds(self) -> int:
        ...
        for addr in live_seeds:
            ...
            for j in range(idx, min(idx + 2000, ni)):
                cur = self.instructions[j]
                if nxt is not None and cur.addr >= nxt:
                    end = nxt
                    break
                if _is_blr(cur):
                    # Multi-return seeded functions (memcpy/strcpy with early
                    # blrs, e.g. func_00D70BFC) keep basic blocks after an early
                    # return; end here only at a real boundary (next is a new
                    # prologue, padding, or the next known start). Otherwise keep
                    # scanning, still bounded by `nxt` so we never sweep into the
                    # next function. Ending at the FIRST blr orphaned those blocks
                    # -> truncated callees corrupted callee-saved r31 + left BSS
                    # registry tables mis-terminated.
                    nx = self.instructions[j + 1] if j + 1 < ni else None
                    if (nx is None or _is_padding(nx)
                            or (nxt is not None and nx.addr >= nxt)
                            or _is_prologue_start(self.instructions, j + 1, ni)):
                        end = cur.addr + 4
                        break
            if end is None:
