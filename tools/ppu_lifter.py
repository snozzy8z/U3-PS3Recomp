...
/* PowerPC integer divide does not raise a host-style divide exception when
 * the divisor is zero (or for INT32_MIN / -1).  The quotient is architecturally
 * undefined in those cases; use zero so generated C never invokes UB. */
static inline int32_t ppc_divw(int32_t a, int32_t b) {
    return (b == 0 || (a == (int32_t)0x80000000u && b == -1)) ? 0 : a / b;
}
...
void     vm_store_vec(uint32_t addr, const void* src);
#ifdef __cplusplus
}
#endif
...
class PPULifter:
    ...
    def _translate_op(self, insn: Instruction, func: LiftedFunction) -> str:
        ...
        if mn in ("divw", "divw."):
            rd, ra, rb = _reg_idx(ops[0]), _reg_idx(ops[1]), _reg_idx(ops[2])
            return f"ctx->gpr[{rd}] = (int64_t)(int32_t)ppc_divw((int32_t)ctx->gpr[{ra}], (int32_t)ctx->gpr[{rb}]);"

        if mn in ("divwu", "divwu."):
            rd, ra, rb = _reg_idx(ops[0]), _reg_idx(ops[1]), _reg_idx(ops[2])
            return f"ctx->gpr[{rd}] = (int64_t)(int32_t)ppc_divwu((uint32_t)ctx->gpr[{ra}], (uint32_t)ctx->gpr[{rb}]);"
        ...
        if mn in load_map:
            ...
            if disp is not None:
                # D-form rA=0 means LITERAL 0 (not gpr[0]). PPC special case.
                addr = f"ctx->gpr[{base}] + {disp}" if base != "0" else f"{disp}"
                expr = f"{helper}({addr})"
        ...
        if mn in store_map:
            ...
            if disp is not None:
                # D-form rA=0 means LITERAL 0 (not gpr[0]). PPC special case.
                addr = f"ctx->gpr[{base}] + {disp}" if base != "0" else f"{disp}"
                line = f"{helper}({addr}, ctx->gpr[{rs_i}]);"
                # Handle update forms
                if mn.endswith("u"):
                    line += f" ctx->gpr[{base}] += {disp};"
                return line
        ...
        if mn in ("lfsx", "lfsux", "lfdx", "lfdux"):
            ...
            ea = f"(ctx->gpr[{ra_i}] + ctx->gpr[{rb_i}])" if ra_i != "0" else f"ctx->gpr[{rb_i}]"
            update = mn.endswith("ux")
            if "s" in mn:
                if update:
                    return (f"{{ uint64_t ea = ctx->gpr[{ra_i}] + ctx->gpr[{rb_i}]; "
                            f"uint32_t tmp = vm_read32(ea); float ftmp; memcpy(&ftmp, &tmp, 4); "
                            f"ctx->fpr[{frd}] = ftmp; ctx->gpr[{ra_i}] = ea; }}")
        ...
        if mn in ("stfsx", "stfsux", "stfdx", "stfdux"):
            ...
            if mn.startswith("stfs"):
                if update:
                    return (f"{{ uint64_t ea = ctx->gpr[{ra_i}] + ctx->gpr[{rb_i}]; "
                            f"float ftmp = (float)ctx->fpr[{frs}]; uint32_t tmp; memcpy(&tmp, &ftmp, 4); "
                            f"vm_write32(ea, tmp); ctx->gpr[{ra_i}] = ea; }}")
                return f"{{ float ftmp = (float)ctx->fpr[{frs}]; uint32_t tmp; memcpy(&tmp, &ftmp, 4); vm_write32({ea}, tmp); }}"
            else:
                if update:
                    return (f"{{ uint64_t ea = ctx->gpr[{ra_i}] + ctx->gpr[{rb_i}]; "
                            f"uint64_t tmp; memcpy(&tmp, &ctx->fpr[{frs}], 8); "
                            f"vm_write64(ea, tmp); ctx->gpr[{ra_i}] = ea; }}")
        ...
        if mn == "bctr":
            # bctr is a computed jump, commonly used for switch dispatch.
            # Its target's nonvolatile-register changes must remain visible.
            return "ps3_indirect_jump(ctx); return;"
        ...
        if mn in ("lfs", "lfsu", "lfd", "lfdu"):
            ...
            disp, base = _disp_base(ops[1])
            if disp is not None:
                update = mn.endswith("u")
                if "s" in mn:
                    if update:
                        return (f"{{ uint64_t ea = ctx->gpr[{base}] + {disp}; "
                                f"uint32_t tmp = vm_read32(ea); float ftmp; memcpy(&ftmp, &tmp, 4); "
                                f"ctx->fpr[{frd}] = ftmp; ctx->gpr[{base}] = ea; }}")
        ...
        if mn in ("stfs", "stfsu", "stfd", "stfdu"):
            ...
            if disp is not None:
                ...
                if mn.startswith("stfs"):
                    if update:
                        return (f"{{ uint64_t ea = ctx->gpr[{base}] + {disp}; "
                                f"float ftmp = (float)ctx->fpr[{frs}]; uint32_t tmp; "
                                f"memcpy(&tmp, &ftmp, 4); vm_write32(ea, tmp); ctx->gpr[{base}] = ea; }}")
                    ...
                    if update:
                        return (f"{{ uint64_t ea = ctx->gpr[{base}] + {disp}; "
                                f"uint64_t tmp; memcpy(&tmp, &ctx->fpr[{frs}], 8); "
                                f"vm_write64(ea, tmp); ctx->gpr[{base}] = ea; }}")
    ...
    def _function_def_lines(self, func, func_by_addr, sorted_addrs,
        ...
        lines.append(f"void {func.name}(ppu_context* ctx) {{")
        # (Runaway-recursion bounding is done centrally in ps3_indirect_call at
        # runtime — a per-function RAII guard here exploded MSVC compile time on
        # the largest generated TUs.)
...
def discover_jump_tables(all_insns, read_u32, toc, text_lo, text_hi):
    ...
    def mem_operand(op):
        try:
            disp, base = op.split('(', 1)
            return int(disp.strip(), 0), base.rstrip(')').strip()
        except (ValueError, TypeError):
            return None

    def find_lwz_def(reg, insns):
        for insn in reversed(insns):
            if insn.mnemonic != 'lwz':
                continue
            operands = [x.strip() for x in insn.operands.split(',')]
            if len(operands) == 2 and operands[0] == reg:
                mem = mem_operand(operands[1])
                if mem is not None:
                    return insn, mem
        return None

    def resolve_loaded_pointer(reg, insns):
        """Resolve `reg = *(r2+disp)` or one TOC-derived base indirection."""
        definition = find_lwz_def(reg, insns)
        if definition is None:
            return None
        load, (disp, base_reg) = definition
        if base_reg == 'r2':
            return read_u32((toc + disp) & 0xFFFFFFFF)

        parent = find_lwz_def(
            base_reg, [insn for insn in insns if insn.addr < load.addr])
        if parent is None:
            return None
        _parent_load, (parent_disp, parent_base) = parent
        if parent_base != 'r2':
            return None
        parent_addr = read_u32((toc + parent_disp) & 0xFFFFFFFF)
        if parent_addr is None:
            return None
        return read_u32((parent_addr + disp) & 0xFFFFFFFF)
    ...
    for i in range(n):
        if all_insns[i].mnemonic != 'bctr':
            continue
        # Optimized switch setup can hoist a TOC-derived base into a large
        # function prologue well before the bounds check and indexed load. UC3
        # has dispatchers whose parent TOC load is more than 64 instructions
        # back, so retain a wider bounded history for the two-load chain.
        win = all_insns[max(0, i - 256):i]
        ...
        before_load = [w for w in win if w.addr < lwzx.addr]
        candidates = []
        for reg in (r_a, r_b):
            if reg not in candidates:
                candidates.append(reg)
        # The index/offset register is commonly also the lwzx destination.
        # Prefer the other input, but still try both for unusual encodings.
        candidates.sort(key=lambda reg: reg == r_val)

        r_base = None
        table_base = None
        for candidate in candidates:
            resolved = resolve_loaded_pointer(candidate, before_load)
            if resolved is not None:
                r_base, table_base = candidate, resolved
                break
        if r_base is None:
            continue

        # Offset table iff an `add rC, *, r_base` combines loaded value + base.
        is_offset = any(
            w.mnemonic == 'add' and
