...
Usage:
    python spu_lifter.py <input_bin> [--base ADDR] [--output dir]
                         [--functions FILE] [--symbol-prefix PREFIX]
"""
...
class SPULifter:
    ...
    def lift_function(self, insns: list[SPUInstruction],
        ...
        if (last_insn is not None and last_insn.mnemonic not in _TERMINATORS
            ...
            func.body_lines.append(
                f"    {{ ctx->pc = 0x{end:X}; {self._func_name(end)}(ctx); return; }}")
    ...
    def _uncond_branch(self, tgt, func: LiftedFunction) -> str:
        ...
        self.branch_targets.add(tgt)
        return f"{{ ctx->pc = 0x{tgt:X}; {self._func_name(tgt)}(ctx); return; }}"
    ...
    def emit_header(self) -> str:
        ...
        for t in sorted(self.call_targets | self.branch_targets):
            if t not in defined:
                lines.append(f"void {self._func_name(t)}(spu_context* ctx); /* external */")
        lines.append("")
        ...
        lines.append(f"void {self._register_name()}(void);")
        lines.append("")
        return "\n".join(lines)

    def emit_source(self, header_name: str = "spu_recomp.h") -> str:
        ...
        if externs:
            ...
            for t in externs:
                lines.append(f"void {self._func_name(t)}(spu_context* ctx) {{")
...
def main() -> None:
    ...
    p.add_argument("--symbol-prefix", default="",
                   help="Prefix every emitted function and registration symbol "
                        "so multiple overlapping SPU images can be linked")
