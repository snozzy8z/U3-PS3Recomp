...
static void host_path(char* out, size_t cap, const char* guest)
{
    ...
    static int title_layout = -1;
    if (title_layout < 0) {
        char usrdir[1100];
        struct stat st;
        snprintf(usrdir, sizeof usrdir, "%s/USRDIR", ppu_vfs_root);
        title_layout = stat(usrdir, &st) == 0 && (st.st_mode & S_IFDIR);
    }

    /* The disc root can itself live at <hdd0>/game/<title>. Mounting
     * /dev_hdd0 below that title root duplicates "game/<title>" and breaks
     * game-data creation. Recover the hdd0 mount root from the title path. */
    static const char hdd0_mount[] = "/dev_hdd0/";
    if (strncmp(guest, hdd0_mount, sizeof hdd0_mount - 1) == 0 ||
        strcmp(guest, "/dev_hdd0") == 0) {
        char hdd0_root[1100];
        snprintf(hdd0_root, sizeof hdd0_root, "%s", ppu_vfs_root);
        for (char* p = hdd0_root; *p; p++) if (*p == '\\') *p = '/';
        if (char* game = strstr(hdd0_root, "/game/")) {
            *game = 0;
        } else {
            snprintf(hdd0_root, sizeof hdd0_root, "%s/dev_hdd0", ppu_vfs_root);
        }
        const char* child = strncmp(guest, hdd0_mount, sizeof hdd0_mount - 1) == 0 ?
                            guest + sizeof hdd0_mount - 1 : "";
        snprintf(out, cap, "%s/%s", hdd0_root, child);
        for (char* p = out; *p; p++) if (*p == '\\') *p = '/';
        return;
    }

    /* Extracted games commonly use <title>/USRDIR without a PS3_GAME
     * directory. Keep supporting the standard disc layout as well. */
    static const char bdvd_game[] = "/dev_bdvd/PS3_GAME/";
    if (title_layout && strncmp(guest, bdvd_game, sizeof bdvd_game - 1) == 0) {
        rel = guest + sizeof bdvd_game - 1;
    } else if (title_layout && strncmp(guest, "/app_home/", 10) == 0) {
        snprintf(out, cap, "%s/USRDIR/%s", ppu_vfs_root, guest + 10);
        for (char* p = out; *p; p++) if (*p == '\\') *p = '/';
        return;
    }
    ...
    if (rel == guest) {
        for (size_t i = 0; i < sizeof(mounts)/sizeof(mounts[0]); i++) {
            ...
            /* FIOS probes the mount itself ("/dev_bdvd") before opening
             * children. Treat the same name without its trailing slash as the
             * VFS root instead of mapping it to <root>/dev_bdvd. */
            if (n > 1 && strlen(guest) == n - 1 &&
                strncmp(guest, mounts[i], n - 1) == 0) {
                rel = guest + n - 1;
                break;
            }
        }
...
static void cellFsOpen(ppu_context* ctx)
{
    ...
    if (hfd < 0) {
        ...
        if (strstr(gpath, "(null)")) ppu_dump_guest_caller((uint32_t)ctx->gpr[3]);
...
/* CellFsStat (big-endian, packed, 0x34 bytes): mode(4), uid(4), gid(4),
...
static void cellFsStat(ppu_context* ctx)
{
    ...
    struct stat st;
    if (stat(hpath, &st) != 0) {
        ctx->gpr[3] = (uint64_t)(int64_t)CELL_FS_ENOENT;
        return;
    }
