...
extern "C"
#endif
void ppu_watch_hit(unsigned int addr, unsigned long long val);
#define PPU_WATCH_ADDR 0xFFFFFFFFu

static inline void vm_store_vec(uint32_t ea, const void* src)
{
    if (ea <= PPU_WATCH_ADDR && PPU_WATCH_ADDR < ea + 16)
        ppu_watch_hit(ea, *(const unsigned long long*)src);
    memcpy((unsigned char*)vm_base + ea, src, 16);
}

static inline void vm_write8(uint32_t addr, uint8_t val)
{
    if (addr == PPU_WATCH_ADDR) ppu_watch_hit(addr, val);
    *vm_ptr8(addr) = val;
}

static inline void vm_write16(uint32_t addr, uint16_t val)
{
    if (addr <= PPU_WATCH_ADDR && PPU_WATCH_ADDR < addr + 2) ppu_watch_hit(addr, val);
...
{
    if (addr <= PPU_WATCH_ADDR && PPU_WATCH_ADDR < addr + 4) ppu_watch_hit(addr, val);
...
{
    if (addr <= PPU_WATCH_ADDR && PPU_WATCH_ADDR < addr + 8) ppu_watch_hit(addr, val);
