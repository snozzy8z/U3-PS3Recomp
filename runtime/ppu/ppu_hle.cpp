...
extern "C" void ps3_hle_call(uint32_t nid, ppu_context* ctx)
{
    ...
    if (!e || !e->handler) {
        /* Log each DISTINCT unresolved NID once (a flat 40-call cap used to hide
         * later NIDs once a high-frequency one like sys_prx_get_module_info ate
         * the budget — which masked that sys_ppu_thread_create was unresolved). */
        static uint32_t seen[128]; static int nseen = 0;
        bool known = false;
        for (int k = 0; k < nseen; k++) if (seen[k] == nid) { known = true; break; }
        if (!known && nseen < 128) {
            seen[nseen++] = nid;
            fprintf(stderr, "[hle] unresolved NID 0x%08X (distinct #%d)\n", nid, nseen);
        }
