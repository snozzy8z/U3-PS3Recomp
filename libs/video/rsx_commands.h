...
#define RSX_MAX_INLINE_VERTS      16384  /* immediate-mode vertex accumulation */

/* Inline immediate-mode vertex data (per-attribute, between BEGIN/END).
 *   SET_VERTEX_DATA2F_M: 0x1880 + attr*8  (2 floats)
 *   SET_VERTEX_DATA4F_M: 0x1C00 + attr*16 (4 floats) */
#define NV4097_SET_VERTEX_DATA2F_M  0x1880
#define NV4097_SET_VERTEX_DATA4F_M  0x1C00
...
typedef struct rsx_state {
    ...
    /* Vertex program instruction memory — written by NV4097_SET_TRANSFORM_PROGRAM.
     * Each instruction is 4 x u32 (D0..D3), up to 512 instructions. The game
     * writes these sequentially starting at transform_program_load. */
    u32 vp_instructions[512][4];
    u32 vp_instruction_count;
    int vp_dirty;
    ...
    /* Inline (immediate-mode) vertex data — NV4097_SET_VERTEX_DATA{2,4}F_M.
     * Many UI/menu draws submit vertices inline between BEGIN/END instead of via
     * vertex arrays + DRAW_ARRAYS. We accumulate per-attribute scratch, commit a
     * vertex when attribute 0 (position) is written, and emit a draw at END. */
    float inline_attr[RSX_MAX_VERTEX_ATTRIBS][4]; /* current-vertex scratch */
    float inline_verts[RSX_MAX_INLINE_VERTS][8];  /* committed: pos.xyzw + col.rgba */
    u32   inline_vert_count;
} rsx_state;
...
typedef struct rsx_backend {
    ...
    /* Immediate-mode draw: state->inline_verts[0..inline_vert_count) hold
     * interleaved pos.xyzw + col.rgba vertices to draw as `primitive`. */
    void (*draw_inline)(void* userdata, u32 primitive, const rsx_state* state);
