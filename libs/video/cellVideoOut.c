...
s32 cellVideoOutGetState(u32 videoOut, u32 deviceIndex, CellVideoOutState* state)
{
    ...
    if (videoOut == CELL_VIDEO_OUT_PRIMARY) {
        u16 refresh_rate = CELL_VIDEO_OUT_REFRESH_RATE_59_94HZ;
        state->state      = CELL_VIDEO_OUT_OUTPUT_STATE_ENABLED;
        state->colorSpace = CELL_VIDEO_OUT_COLOR_SPACE_RGB;
        state->displayMode.resolutionId = s_resolution_id;
        state->displayMode.scanMode = CELL_VIDEO_OUT_SCAN_MODE_PROGRESSIVE;
        state->displayMode.aspect = s_aspect;
        if (s_resolution_id == CELL_VIDEO_OUT_RESOLUTION_576)
            refresh_rate = CELL_VIDEO_OUT_REFRESH_RATE_50HZ;
        state->displayMode.refreshRates = ps3_bswap16(refresh_rate);
    } else {
