...
static void null_draw_inline(void* ud, u32 primitive, const rsx_state* state)
{
    (void)ud;
    static int s_log = 0;
    if (s_log < 20) {
        printf("[RSX null] draw_inline(prim=%u, verts=%u) first pos=(%.3f,%.3f)\n",
               primitive, state->inline_vert_count,
               state->inline_vert_count ? state->inline_verts[0][0] : 0.0f,
               state->inline_vert_count ? state->inline_verts[0][1] : 0.0f);
        s_log++;
    }
}
