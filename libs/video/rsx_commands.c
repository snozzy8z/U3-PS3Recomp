...
int rsx_process_method(rsx_state* state, u32 method, u32 data)
{
    ...
    /* NV4097_SET_TRANSFORM_PROGRAM — vertex program instruction upload.
     * Games write 4 consecutive dwords (D0..D3) per instruction, starting at
     * the slot set by SET_TRANSFORM_PROGRAM_LOAD. The method range at 0x0B80
     * spans 128 dwords (32 instructions × 4 words). Multiple commands fill
     * consecutive instruction slots. */
    if (method >= NV4097_SET_TRANSFORM_PROGRAM &&
        method <  NV4097_SET_TRANSFORM_PROGRAM + 128 * 4) {
        u32 word_offset = (method - NV4097_SET_TRANSFORM_PROGRAM) / 4;
        u32 instr_idx = state->transform_program_load + (word_offset >> 2);
        u32 word_lane = word_offset & 3;
        if (instr_idx < 512) {
            state->vp_instructions[instr_idx][word_lane] = data;
            if (instr_idx + 1 > state->vp_instruction_count)
                state->vp_instruction_count = instr_idx + 1;
            state->vp_dirty = 1;
            state->shader_dirty = 1;
        }
        return 0;
    ...
    /* Inline immediate-mode vertex data (SET_VERTEX_DATA2F_M / 4F_M).
     * Accumulate per-attribute; committing a vertex when attribute 0's last
     * component is written (position is the per-vertex latch on RSX). */
    if (method >= NV4097_SET_VERTEX_DATA2F_M &&
        method < NV4097_SET_VERTEX_DATA2F_M + RSX_MAX_VERTEX_ATTRIBS * 8) {
        u32 off  = method - NV4097_SET_VERTEX_DATA2F_M;
        u32 attr = off / 8, comp = (off % 8) / 4;
        float f; memcpy(&f, &data, 4);
        state->inline_attr[attr][comp] = f;
        if (attr == 0 && comp == 1 &&
            state->inline_vert_count < RSX_MAX_INLINE_VERTS) {
            float* v = state->inline_verts[state->inline_vert_count++];
            v[0] = state->inline_attr[0][0]; v[1] = state->inline_attr[0][1];
            v[2] = 0.0f;                     v[3] = 1.0f;            /* pos.zw */
            v[4] = state->inline_attr[1][0]; v[5] = state->inline_attr[1][1]; /* texcoord uv */
            v[6] = state->inline_attr[2][0]; v[7] = state->inline_attr[2][3]; /* col.r, col.a */
        }
        return 0;
    }
    if (method >= NV4097_SET_VERTEX_DATA4F_M &&
        method < NV4097_SET_VERTEX_DATA4F_M + RSX_MAX_VERTEX_ATTRIBS * 16) {
        u32 off  = method - NV4097_SET_VERTEX_DATA4F_M;
        u32 attr = off / 16, comp = (off % 16) / 4;
        float f; memcpy(&f, &data, 4);
        state->inline_attr[attr][comp] = f;
        if (attr == 0 && comp == 3 &&
            state->inline_vert_count < RSX_MAX_INLINE_VERTS) {
            float* v = state->inline_verts[state->inline_vert_count++];
            v[0] = state->inline_attr[0][0]; v[1] = state->inline_attr[0][1];
            v[2] = state->inline_attr[0][2]; v[3] = state->inline_attr[0][3];
            v[4] = state->inline_attr[2][0]; v[5] = state->inline_attr[2][1];
            v[6] = state->inline_attr[2][2]; v[7] = state->inline_attr[2][3];
        }
        return 0;
    }
    ...
    if (method == NV4097_SET_BEGIN_END) {
        if (data != 0) {
            ...
            state->inline_vert_count = 0;   /* reset inline accumulation */
            ...
            /* Emit accumulated immediate-mode geometry at END. */
            if (state->inline_vert_count > 0 && s_backend && s_backend->draw_inline)
                s_backend->draw_inline(s_backend->userdata,
                                       state->primitive_type, state);
            state->inline_vert_count = 0;
        }
        return 0;
    }
