...
static char s_checked_dir[64] = "";

static int  s_boot_checked = 0;
static u32  s_game_type = CELL_GAME_GAMETYPE_DISC;
static u32  s_check_type = CELL_GAME_GAMETYPE_DISC;
                       ...
                       char* dirName)
{
    ...
    s_boot_checked = 1;
    s_check_type = CELL_GAME_GAMETYPE_DISC;
    s_checked_dir[0] = '\0';
...
s32 cellGameContentPermit(char* contentInfoPath, char* usrdirPath)
{
    ...
    if (s_check_type == CELL_GAME_GAMETYPE_GAMEDATA && s_checked_dir[0]) {
        if (contentInfoPath) {
            snprintf(contentInfoPath, CELL_GAME_PATH_MAX,
                     "/dev_hdd0/game/%s", s_checked_dir);
        }
        if (usrdirPath) {
            snprintf(usrdirPath, CELL_GAME_PATH_MAX,
                     "/dev_hdd0/game/%s/USRDIR", s_checked_dir);
        }
        return CELL_OK;
    }

    if (contentInfoPath) {
        snprintf(contentInfoPath, CELL_GAME_PATH_MAX, "/dev_bdvd/PS3_GAME");
    }

    if (usrdirPath) {
        snprintf(usrdirPath, CELL_GAME_PATH_MAX, "/dev_bdvd/PS3_GAME/USRDIR");
    }
...
s32 cellGameDataCheck(u32 type, const char* dirName, CellGameContentSize* size)
{
    ...
    s_check_type = type;
    strncpy(s_checked_dir, check_dir, sizeof(s_checked_dir) - 1);
    s_checked_dir[sizeof(s_checked_dir) - 1] = '\0';
...
s32 cellGameGetParamString(s32 id, char* buf, u32 bufsize)
{
    ...
    case CELL_GAME_PARAMID_VERSION:
        strncpy(buf, "01.00", bufsize - 1);
        buf[bufsize - 1] = '\0';
        break;
    case CELL_GAME_PARAMID_PS3_SYSTEM_VER:
        strncpy(buf, "03.6000", bufsize - 1);
        buf[bufsize - 1] = '\0';
        break;
    default:
...
{
    printf("[cellGame] SetParamString(id=%d, value='%s')\n",
           id, buf ? buf : "<null>");
    if (!buf)
        return CELL_GAME_ERROR_PARAM;
    return CELL_OK;
                           ...
                           char* tmpContentInfoPath, char* tmpUsrdirPath)
{
    ...
    if (!init || s_check_type != CELL_GAME_GAMETYPE_GAMEDATA || !s_checked_dir[0])
        return CELL_GAME_ERROR_PARAM;

    char path[CELL_GAME_PATH_MAX];
    snprintf(path, sizeof(path), "%s/%s", s_content_path, s_checked_dir);
    ...
    if (tmpContentInfoPath) {
        snprintf(tmpContentInfoPath, CELL_GAME_PATH_MAX,
                 "/dev_hdd0/game/%s", s_checked_dir);
    }

    if (tmpUsrdirPath) {
        snprintf(tmpUsrdirPath, CELL_GAME_PATH_MAX,
                 "/dev_hdd0/game/%s/USRDIR", s_checked_dir);
    }
