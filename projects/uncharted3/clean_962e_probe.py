...
patched = b"""void func_00962EA0(ppu_context* ctx) {\r
        {\r
...
legacy_patched = patched.replace(
    b"call=%u sp=%08X lr=%08X r3=%08X",
    b"call=%u r3=%08X",
).replace(
    b"call, (uint32_t)ctx->gpr[1], (uint32_t)ctx->lr, p,\r\n                        ",
    b"call, p, ",
)
clean = b"""void func_00962EA0(ppu_context* ctx) {\r
        vm_write64(ctx->gpr[1] + -0xF0, ctx->gpr[1]); ctx->gpr[1] += -0xF0;"""

if clean in data:
    print("func_00962EA0 probe already clean")
elif data.count(patched) == 1:
    path.write_bytes(data.replace(patched, clean, 1))
    print("func_00962EA0 probe removed")
elif data.count(legacy_patched) == 1:
    path.write_bytes(data.replace(legacy_patched, clean, 1))
    print("legacy func_00962EA0 probe removed")
else:
    raise SystemExit("expected one func_00962EA0 probe")
